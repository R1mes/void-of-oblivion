...
static func apply_traces(unit: CombatUnit, bm: BattleManager) -> void:
	...
	if is_slot_1:
		...
		bm.log_message("🦢 Талант Сёдзи: Позиция саппорта — активировано постоянное состояние «Танец» (+16%% Скорости, +25%% не-Бинарного урона отряда)!")
		for ally in bm.allies:
			if ally.is_alive():
				ally.add_speed_modifier(0.16, 0.0)
				ally.set_meta("shoji_swan_dance_ally_buff", true)

static func execute_basic_attack(attacker: CombatUnit, target: CombatUnit, bm: BattleManager) -> void:
	var res := bm.calc_dmg(attacker, target, 0.95, 0.0, false, 0.0, 0.0, false, true, "Basic")
...
static func execute_enhanced_basic(attacker: CombatUnit, target: CombatUnit, bm: BattleManager) -> void:
	...
	var res := bm.calc_dmg(attacker, target, 1.25, 0.0, false, 0.0, 0.0, false, true, "Basic")
	...
	for e in adj:
		if e.is_alive():
			var adj_res := bm.calc_dmg(attacker, e, 0.35, 0.0, false, 0.0, 0.0, false, true, "Basic")
	...
	bm.log_message("🦢 %s — Усиленная базовая атака по %s: %d урона (и соседям по 35%% СА). Получено +13 Векторов." % [attacker.display_name, target.display_name, int(main_dmg)])
	bm.action_order_changed.emit()
	bm.unit_updated.emit(attacker)
...
static func execute_skill_q(attacker: CombatUnit, target: CombatUnit, bm: BattleManager) -> void:
	...
	if stance == "virus":
		...
		if target != null and target.is_alive():
			...
			var res := bm.calc_dmg(attacker, target, 1.70, 0.0, false, 0.0, 0.0, false, true, "Binary")
			var dmg: float = float(res.get("damage", 0.0))
			...
			for e in adj:
				if e.is_alive():
					var adj_res := bm.calc_dmg(attacker, e, 0.55, 0.0, false, 0.0, 0.0, false, true, "Binary")
			...
			bm.log_message("🦢 Навык Q Сёдзи (Вирус): %d Бинарного урона по %s и 55%% соседям!" % [int(dmg), target.display_name])
...
static func execute_skill_e(attacker: CombatUnit, bm: BattleManager) -> void:
	...
	for enemy in living:
		var res := bm.calc_dmg(attacker, enemy, 1.15, 0.0, false, 0.0, 0.0, false, true, "Binary")
...
static func execute_ultimate(attacker: CombatUnit, bm: BattleManager) -> void:
	...
	if stance == "virus":
		attacker.set_meta("is_binary_attack", true)
		attacker.set_meta("shoji_swan_ult_virus_bonus", 0.35)
		for enemy in living:
			var res := bm.calc_dmg(attacker, enemy, 1.45, 0.0, false, 0.0, 0.0, false, true, "Binary")
			bm.deal_damage(enemy, float(res.get("damage", 0.0)), attacker, CombatConstants.Element.WIND, bool(res.get("crit", false)), "Binary")
			ToughnessSystem.apply_weakness_hit(attacker, enemy, bm, 2.0)
		attacker.remove_meta("shoji_swan_ult_virus_bonus")
		attacker.remove_meta("is_binary_attack")
		bm.log_message("🦢 СВЕРХСПОСОБНОСТЬ СЁДЗИ (Вирус): 145%% СА Бинарного урона (+35%% бонус урона) по всем врагам!")
	else:
		var cur_vectors: int = bm.get_console_vectors()
		var vuln_pct: float = minf(float(cur_vectors) * 0.0075, 0.50)
		var e1_dmg_mult: float = 1.0

		if attacker.eidolon >= 1:
			vuln_pct = 0.65
			e1_dmg_mult += float(cur_vectors) * 0.0075

		attacker.set_meta("is_binary_attack", true)
		for enemy in living:
			var res := bm.calc_dmg(attacker, enemy, 1.45 * e1_dmg_mult, 0.0, false, 0.0, 0.0, false, true, "Binary")
			bm.deal_damage(enemy, float(res.get("damage", 0.0)), attacker, CombatConstants.Element.WIND, bool(res.get("crit", false)), "Binary")
			ToughnessSystem.apply_weakness_hit(attacker, enemy, bm, 2.0)
		...
		attacker.remove_meta("is_binary_attack")
		bm.log_message("🦢 СВЕРХСПОСОБНОСТЬ СЁДЗИ (Танец): 145%% СА урона по всем врагам, наложена уязвимость ко всем видам урона +%.1f%% на 2 хода!" % [vuln_pct * 100.0])
...
static func apply_technique(enemies: Array, attacker: CombatUnit, bm: BattleManager) -> void:
	...
	for enemy in enemies:
		if enemy is CombatUnit and enemy.is_alive():
			var res := bm.calc_dmg(attacker, enemy, 0.90, 0.0, false, 0.0, 0.0, false, true, "Binary")
			bm.deal_damage(enemy, float(res.get("damage", 0.0)), attacker, CombatConstants.Element.WIND, bool(res.get("crit", false)), "Binary")
			enemy.set_meta("shoji_swan_tech_vuln_turns", 2)
			enemy.set_meta("shoji_swan_tech_vuln_skip_tick", true)
	attacker.remove_meta("is_binary_attack")
	bm.log_message("⚔ Атакующая техника Сёдзи: нанесено 90%% СА Бинарного урона всем врагам, наложена уязвимость ко всему урону +25%% на 2 хода!")

...
static func _get_raw_dynamic_crit_rate(unit: CombatUnit, allies: Array = []) -> float:
	if unit == null:
		return 0.0
...
static func _get_dynamic_crit_rate(unit: CombatUnit, allies: Array = []) -> float:
	if TeamConfig.is_test_mode_battle and unit != null and unit.is_ally:
		return 1.0
	return _get_raw_dynamic_crit_rate(unit, allies)
...
static func get_stats_text(unit: CombatUnit, allies: Array = []) -> String:
	...
	var raw_crit_rate: float = _get_raw_dynamic_crit_rate(unit, allies)

	# --- Расчет эффективного Крит. Урона для Инфо-панели ---
	var effective_crit_dmg: float = s.get_effective_crit_dmg(unit.statuses)
	if TeamConfig.is_test_mode_battle and unit.is_ally:
		dynamic_crit_rate = 1.0
		if raw_crit_rate > 1.0:
			effective_crit_dmg += (raw_crit_rate - 1.0) * 1.5
	...
	# Учет бонуса Крит. урона от сета Восставший Краснодар (+32% при наличии Духа Памяти или призыва)
	if unit.has_meta("has_set_risen_krasnodar") or String(unit.get_meta("planar_set", "")) == "risen_krasnodar":
		var has_memo_or_summon := bool(unit.get_meta("has_summoned_entity", false))
		for a in allies:
			if a is Memosprite and a.is_alive() and a.owner == unit:
				has_memo_or_summon = true
				break
		if has_memo_or_summon:
			effective_crit_dmg += 0.32
...
static func get_statuses_text(unit: CombatUnit, allies: Array = []) -> String:
	...
	if unit.has_meta("bloom_defender_crit_turns") and int(unit.get_meta("bloom_defender_crit_turns", 0)) > 0:
		buffs.append("• Защитница цветущей земли (%d х.): Крит. урон повышен на +30%%." % int(unit.get_meta("bloom_defender_crit_turns", 0)))

	if unit.has_meta("kyle_crit_buff_turns") and int(unit.get_meta("kyle_crit_buff_turns", 0)) > 0:
		buffs.append("• Воодушевление восстания (%d х.): Крит. шанс +30%%, Крит. урон +60%%." % int(unit.get_meta("kyle_crit_buff_turns", 0)))

	if unit.has_meta("kyle_veil_active") and bool(unit.get_meta("kyle_veil_active", false)):
		var v_stacks: int = int(unit.get_meta("kyle_veil_stacks", 0))
		buffs.append("• Завеса восстания x%d: уязвимости заблокированы, получаемый урон снижен на 40%%. Атаки Духов Памяти снимают по 2 стака." % v_stacks)

	if unit.has_meta("insurgent_preparing_riot") and bool(unit.get_meta("insurgent_preparing_riot", false)):
		buffs.append("• Подготовка бунта: готовит атаку «Кровавый бунт». Пробитие уязвимости прервёт подготовку!")
	...
	match active_lc_id:
		...
		"farewell_before_awakening":
			buffs.append("• Прощание перед пробуждением (конус): Эффект пробития повышен на +50%. Атака восстанавливает 8 ед. энергии (1 раз за ход).")
		"last_summer":
			buffs.append("• Последнее лето (конус): Урон бонус-атак повышен на +30%. Накладывает «Закрой глаза» при FUA.")
		"alone_again":
			buffs.append("• И вновь я один (конус): Макс. HP повышено на +30%. При потере HP в свой ход даёт «Искупление» (игнор 30% защиты на 2 хода). Ульт продвигает действие на 12%.")
	...
	if unit.has_meta("redemption_turns") and int(unit.get_meta("redemption_turns", 0)) > 0:
		buffs.append("• Искупление (%d х.) [color=yellow](конус «И вновь я один»)[/color]: игнорирование 30%% защиты врагов." % int(unit.get_meta("redemption_turns", 0)))
	...
	# --- РЕЖИМ «ТЕСТОВАЯ СРЕДА» ---
	if TeamConfig.is_test_mode_battle and unit.is_ally:
		var raw_cr := _get_raw_dynamic_crit_rate(unit, allies)
		if raw_cr > 1.0:
			var excess_cr := (raw_cr - 1.0) * 100.0
			var gained_cd := excess_cr * 1.5
			buffs.append("• 🧪 Тестовая среда: КШ зафиксирован на 100%% (+%.1f%% КШ конвертировано в +%.1f%% КУ)." % [excess_cr, gained_cd])
		else:
			buffs.append("• 🧪 Тестовая среда: Крит. шанс зафиксирован на 100%.")
	...
	if unit.is_ally:
		...
		# 11. Сет: Перебежчик тёмной стороны (4 шт.)
		if unit.has_meta("set_dark_side_defector_4"):
			var cur_be: float = unit.get_effective_be()
			if cur_be >= 1.80:
				buffs.append("• Перебежчик тёмной стороны (4 шт.): ЭП >= 180%% (сейчас %.0f%%) -> урон пробития игнорирует 10%% защиты, суперпробитие игнорирует 15%% защиты." % [cur_be * 100.0])
			elif cur_be >= 1.30:
				buffs.append("• Перебежчик тёмной стороны (4 шт.): ЭП >= 130%% (сейчас %.0f%%) -> урон пробития игнорирует 10%% защиты. При ЭП >= 180%% суперпробитие будет игнорировать 15%% защиты." % [cur_be * 100.0])
			else:
				buffs.append("• Перебежчик тёмной стороны (4 шт.): ЭП < 130%% (сейчас %.0f%%) -> требуется >= 130%% ЭП для 10%% игнорирования защиты пробитием." % [cur_be * 100.0])

		# 12. Сет: Защитница цветущей земли
		if unit.has_meta("set_bloom_defender_2"):
			var has_memo := false
			for a in allies:
				if a is Memosprite and a.is_alive() and a.owner == unit:
					has_memo = true
					break
			if has_memo:
				buffs.append("• Защитница цветущей земли (2 шт.): Дух Памяти активен -> Скорость повышена на +6%%.")
			else:
				buffs.append("• Защитница цветущей земли (2 шт.): Требуется активный Дух Памяти для бонуса Скорости +6%%.")

		# 13. Сет: Восставший Краснодар [Планарный сет]
		if unit.has_meta("has_set_risen_krasnodar") or String(unit.get_meta("planar_set", "")) == "risen_krasnodar":
			var has_memo_or_summon := bool(unit.get_meta("has_summoned_entity", false))
			for a in allies:
				if a is Memosprite and a.is_alive() and a.owner == unit:
					...
					break
			if has_memo_or_summon:
				buffs.append("• Восставший Краснодар [Планарный сет]: Дух Памяти/призыв на поле -> Крит. урон повышен на +32%% (суммарно +48%% КУ).")
			else:
				buffs.append("• Восставший Краснодар [Планарный сет]: Требуется Дух Памяти или призванное существо для доп. бонуса +32%% КУ (+16%% КУ базово).")

		# 14. Сет: Замурованный под руинами штаб [Планарный сет]
		if unit.has_meta("has_set_ruins_hq") or String(unit.get_meta("planar_set", "")) == "ruins_hq":
			var cur_spd: float = unit.stats.get_effective_spd()
			if cur_spd >= 145.0:
				buffs.append("• Замурованный под руинами штаб [Планарный сет]: Скорость >= 145 (сейчас %d) -> Эффект пробития повышен ещё на +20%% (суммарно +36%% ЭП)." % int(cur_spd))
			else:
				buffs.append("• Замурованный под руинами штаб [Планарный сет]: Скорость < 145 (сейчас %d) -> требуется >= 145 Скорости для +20%% ЭП (+16%% ЭП базово)." % int(cur_spd))
	...
	if unit.id == "antimatter_paws" or (unit is Memosprite and (unit as Memosprite).definition != null and (unit as Memosprite).definition.id == "antimatter_paws"):
		...
		buffs.append("• Заряды Лап: %d/4 (макс. ХП +%d%%)." % [ch, (ch - 1) * 20])
		var p_t3 := int(unit.get_meta("paws_trace3_stacks", 0))
		if p_t3 > 0:
			buffs.append("• След 3 Лап: Наносимый урон +%d%% (стаков: %d/6)." % [p_t3 * 5, p_t3])
	...
	if unit.has_meta("close_eyes_turns") and int(unit.get_meta("close_eyes_turns", 0)) > 0:
		debuffs.append("• Закрой глаза (%d х.) [color=yellow](конус «Последнее лето»)[/color]: получаемый урон бонус-атак повышен на +30%%." % int(unit.get_meta("close_eyes_turns", 0)))
	if unit.has_meta("close_eyes_perm_fua_vuln") and float(unit.get_meta("close_eyes_perm_fua_vuln", 0.0)) > 0.0:
		var p_vuln: float = float(unit.get_meta("close_eyes_perm_fua_vuln", 0.0)) * 100.0
		debuffs.append("• Эхо лета [color=yellow](конус «Последнее лето»)[/color]: получаемый урон бонус-атак повышен на +%.0f%% (макс 48%%)." % p_vuln)
	...
	if unit.has_meta("sanguinia_special_guest_charges"):
		...
		debuffs.append("• Особый гость (%d/12 зар.)%s: получаемый урон бонус-атак повышен на 70%%; атаки союзников снижают заряды на 1. На 9, 6, 3, 0 Сангиния немедленно проводит Бонус-атаку (130%% центру, 35%% соседям). При 0 статус снимается и восстанавливается 1 Очко Навыков." % [
	...
	if unit.has_meta("kyle_suppression_turns") and int(unit.get_meta("kyle_suppression_turns", 0)) > 0:
		debuffs.append("• Подавление мятежа (%d х.): наносимый урон снижен на 30%%." % int(unit.get_meta("kyle_suppression_turns", 0)))

	if unit.has_meta("kyle_mind_fracture_turns") and int(unit.get_meta("kyle_mind_fracture_turns", 0)) > 0:
		var mf_stacks: int = int(unit.get_meta("kyle_mind_fracture_stacks", 0))
		debuffs.append("• Раскол разума x%d (%d х.): получаемый урон увеличен на +%d%%." % [mf_stacks, int(unit.get_meta("kyle_mind_fracture_turns", 0)), mf_stacks * 15])

	if unit.has_meta("insurgent_broken_spirit_turns") and int(unit.get_meta("insurgent_broken_spirit_turns", 0)) > 0:
		debuffs.append("• Сломленный дух (%d х.): Защита -20%%, получаемый урон Пробития и Суперпробития +100%%." % int(unit.get_meta("insurgent_broken_spirit_turns", 0)))
...
static func get_skills_text(unit: CombatUnit) -> String:
	...
	match unit.id:
		...
		"antimatter_slave":
			return _antimatter_slave_skills()
		"insurgent":
			return _insurgent_skills()
		"kyle_rebel_leader":
			return _kyle_rebel_leader_skills()
...
static func _antimatter_slave_skills() -> String:
	return "👾 [color=red]РАБ АНТИМАТЕРИИ (ОБЫЧНЫЙ ВРАГ)[/color] — ХП: 30 000 | Стойкость: 90\n\n" + \
	"Уязвимости: Ветряная, Огненная.\n" + \
	"Тактика: Пробитие, Духи Памяти.\n\n" + \
	"[color=yellow]Способности:[/color]\n" + \
	"⚔ Клеймо энтропии: Наносит 100% СА выбранному союзнику.\n" + \
	"🌌 Аннигиляционный импульс: Наносит 60% СА всем союзникам.\n\n" + \
	"[color=cyan]Особенности тактики:[/color]\n" + \
	"💥 Коллапс пробития: При пробитии уязвимости взрывается, нанося другим противникам урон в размере 15% от их макс. ХП и снижая их стойкость на 30 ед.\n" + \
	"🌸 Реакция на Духа Памяти: Удары Духа Памяти снимают на 50% больше стойкости и продвигают действие владельца Духа на 10%!"

static func _insurgent_skills() -> String:
	return "⚔ [color=orange]ВОССТАВШИЙ (ЭЛИТНЫЙ ВРАГ)[/color] — ХП: 225 000 | Стойкость: 240\n\n" + \
	"Уязвимости: Мнимая, Электрическая, Квантовая.\n" + \
	"Тактика: Пробитие уязвимости.\n\n" + \
	"[color=yellow]Способности:[/color]\n" + \
	"⚔ Яростный взмах: Наносит 120% СА выбранному союзнику.\n" + \
	"⚡ Подавление мятежа: Наносит 80% СА цели и 40% СА соседям.\n" + \
	"🩸 Подготовка бунта: Переходит в режим концентрации на 1 ход. В следующий ход обрушит «Кровавый бунт» (180% СА по всем союзникам).\n\n" + \
	"[color=cyan]Особенности тактики:[/color]\n" + \
	"💥 Срыв подготовки: Пробитие уязвимости во время подготовки прерывает атаку, откладывает действие врага на 40% и накладывает статус «Сломленный дух» (Защита -20%, получаемый урон Пробития и Суперпробития +100%) на 2 хода!"

static func _kyle_rebel_leader_skills() -> String:
	return "👑 [color=red]КАЙЛ • ЛИДЕР ВОССТАНИЯ (БОСС — 2 ФАЗЫ)[/color]\n\n" + \
	"Уязвимости: Ветряная, Квантовая, Физическая.\n" + \
	"Тактика: Духи Памяти и Пробитие уязвимости.\n\n" + \
	"[color=yellow]ФАЗА 1 (ХП: 350 000 | Стойкость: 360):[/color]\n" + \
	"⚔ Меч возмездия: 130% СА одной цели.\n" + \
	"🌪 Волна восстания: 75% СА всем союзникам.\n" + \
	"🛡 Завеса восстания: Окружает себя щитом (6 стаков): блокирует уязвимости, входящий урон -40%. Атаки Духов Памяти снимают по 2 стака за удар!\n" + \
	"⚡ Казнь тиранов: 220% СА по одной цели. Если на поле есть Дух Памяти, он блокирует 70% урона без вреда для себя, а Кайл получает отдачу (теряет 40 стойкости и задерживает ход на 20%)!\n\n" + \
	"[color=yellow]ФАЗА 2 (ХП: 400 000 | Стойкость: 420):[/color]\n" + \
	"⚔ Кровавый рассвет: 160% СА цели и 80% соседям.\n" + \
	"💥 Неукротимый мятеж: 110% СА по всем союзникам, вешает «Подавление мятежа» (-30% наносимого урона на 2 хода).\n" + \
	"🔥 Воодушевление восстания: Кайл повышает свой Крит. шанс на 30% и Крит. урон на 60% на 2 хода.\n" + \
	"⚡ Казнь тиранов (Усиленная): 280% СА цели с наивысшей СА. Дух Памяти по-прежнему блокирует 70% урона, а Кайл теряет 60 стойкости и задерживает ход на 30%!\n\n" + \
	"[color=cyan]Особенности взаимодействия:[/color]\n" + \
	"🌸 Духи Памяти: Каждая атака Духа снимает 2 стака Завесы, накладывает «Раскол разума» (+15% получаемого урона кайлом, до 3 стаков), восстанавливает 5 энергии всей команде и снимает 20 доп. стойкости!\n" + \
	"🎯 Пробитие: Команды на Пробитие получают полные бонусы урона, а при пробитии стойкости Кайл теряет все защитные статусы."
...
static func _antimatter_paws_skills() -> String:
	...
	"Базовая скорость: 165. Макс. ХП = 120% макс. ХП Раймса (+20% за каждый заряд выше 1). Перенаправляет смертельный урон с союзников на себя с множителем 300%. Недоступен для прямой атаки врагов.\n\n" + \
	...
	"  • 3 удара по 30% макс. ХП Лап всем врагам + 4-й удар 45% макс. ХП Лап всем врагам. При наличии >= 60 Зеро тратит 20 Зеро и повторяет 4-й удар (45%). Сбрасывает заряды до 1.\n\n" + \
...
static func _rimes_ascension_skills(eidolon: int) -> String:
	...
	"🔷 [b]Навык Q (0 ОН) — Слияние [Групповая атака][/b]: Без Лап наносит 90% макс. ХП Раймса всем врагам. При наличии Лап расходует 25% текущего ХП союзников (35% у Лап) и выполняет совместную атаку (50% макс. ХП Раймса + 30% макс. ХП Лап). При Зеро >= 60 тратит 10 Зеро, увеличивает урон на 20% и восстанавливает Лапам 1 заряд.\n\n" + \
	...
	"💡 [b]Талант — Симфония Антиматерии[/b]: Не накапливает обычную энергию. За каждый 1% потерянного союзниками ХП получает 0.2% Крещендо (шкала до 100%). При потере ХП союзником наносимый урон Раймса и Лап повышается на 20% на 3 хода (стакается до 3 раз, макс +60%).\n\n" + \
	"⚡ [b]Следы[/b]:\n" + \
	...
	"  • След 3: За каждое применение/удар Лап их наносимый урон повышается на 5% до конца хода Лап (до 6 раз, макс +30%).\n\n" + \
...
static func _sanguinia_skills(eidolon: int) -> String:
	...
	"🔹 [b]Навык E [Ослабление] (2 ОН)[/b]: Помечает выбранного врага статусом «Особый гость» (12 зарядов). Получаемый Особым гостем урон бонус-атак повышается на 70% (пока действует этот статус). Каждая атака союзников (в том числе Бонус-атаки) уменьшает заряды на 1. При 9/6/3/0 зарядах Сангиния проводит Бонус-атаку: 130% СА цели и 35% СА двум соседним целям. При 0 зарядах статус снимается, а отряд восстанавливает 1 Очко Навыков (+30 ЭН).\n\n" + \
	"✨ [b]Сверхспособность (150 ЭН) [Усиление][/b]: Создает на шкале действий «Готовьтесь...» (AV 19) и три «Заселение!» (AV 20). В свой ход «Готовьтесь...» продвигает союзника с наивысшей СА на 100%, дает ему 40 энергии, +60% СА на 1 ход и игнорирование 20% защиты на следующую атаку (След 2). В свой ход «Заселение!» наносят групповую бонус-атаку всем врагам (40% СА Сангинии), уменьшают заряды «Особого гостя» и восстанавливают Сангинии 7 энергии (След 1)." + e4 + e6 + "\n\n" + \
	"💡 [b]Талант — «Журчание волн» [Поддержка][/b]: За каждую союзную Бонус-атаку и за Сверхспособность союзника с наибольшей СА получает 2 стака (макс 47). Каждый стак повышает урон Бонус-атак всех союзников на 2%. Когда количество уровней «Журчание волн» становится равно 47, Сангиния моментально продвигает действие персонажа с самой высокой силой атаки в отряде на 100% и повышает урон, наносимый его следующей атакой на 100%. После этого она принудительно заставляет этого союзника использовать свой Навык Е на противнике с самым высоким текущим ХП без траты очков навыков. Если союзник не может использовать свой Навык Е (например, заморожен или находится под эффектом контроля, либо Навык Е временно недоступен), то как только он снова получает возможность действовать, Сангиния принудительно заставляет его использовать свой Навык Е вне зависимости от того, чей сейчас ход. Только после успешного применения Навыка Е количество уровней «Журчание волн» сбрасывается до 0 (След 3: восстанавливает 1 Очко Навыков)." + e1 + e2 + "\n\n" + \
	"⚡ [b]Следы[/b]:\n" + \
